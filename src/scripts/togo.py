...
if __name__ == "__main__":
    ...
    togo_missing_values = df_togo.isnull().sum()
    togo_missing_values = togo_missing_values[togo_missing_values > 0]
    #print(togo_missing_values)
    # Comment columns empty so we drop them
    df_togo = df_togo.drop(columns=['Comments'])
    # Drop the 'comments' column if it exists
    if 'Comments' in df_togo.columns:
        df_togo = df_togo.drop(columns=['Comments'])
...
for i, col in enumerate(key_cols):
    plt.subplot(4, 3, i+1)
    sns.boxplot(data=df_togo, x=col)
    plt.title(f'Boxplot of {col}')
plt.tight_layout()
#plt.show()
# ------------------------------
# Compute Z-scores to Detect Outliers
z_scores = df_togo[key_cols].apply(zscore)
# Flag rows where |Z| > 3
togo_outlier_mask = z_scores.abs() > 3
togo_outlier_counts = togo_outlier_mask.sum()


# total number of rows flagged as outliers in any key column
togo_outlier_mask.sum()
#create graph for total outlier in the data
plt.figure(figsize=(10, 6))
sns.barplot(x=togo_outlier_counts.index, y=togo_outlier_counts.values)
...
rows_with_outliers = togo_outlier_mask.any(axis=1)

# Drop the outlier rows
df_togo_clean = df_togo[~rows_with_outliers]
#print(f"Total rows after dropping outliers: {len(df_togo_clean)}")

# Verify the data is clean by checking for outliers again
z_scores_clean = df_togo_clean[key_cols].apply(zscore)
togo_clean_outlier_mask = z_scores_clean.abs() > 3
togo_clean_outlier_count = togo_clean_outlier_mask.any(axis=1).sum()
#print(f"Remaining outliers after cleaning: {togo_clean_outlier_count}")

# Implement iterative outlier removal until convergence
#print("\nPerforming iterative outlier removal...")
iteration = 1
max_iterations = 5
previous_count = len(df_togo_clean)

while togo_clean_outlier_count > 0 and iteration <= max_iterations:
    # Remove outliers
    df_togo_clean = df_togo_clean[~togo_clean_outlier_mask.any(axis=1)]
    ...
    # Print results
    current_count = len(df_togo_clean)
    removed = previous_count - current_count
    # print(f"Iteration {iteration}: Removed {removed} rows, {togo_clean_outlier_count} outliers remaining")
    
    # Update for next iteration
    previous_count = current_count
    iteration += 1

#print(f"\nFinal clean dataset: {len(df_togo_clean)} rows")
#print(f"Total rows removed: {len(df_togo) - len(df_togo_clean)}")

# Save the cleaned dataset
clean_file_path = "data/togo-dapaong-clean.csv"
df_togo_clean.to_csv(clean_file_path, index=False)
#print(f"Cleaned data saved to {clean_file_path}")

# # Compare data statistics before and after cleaning
#print("\nStatistics before cleaning:")
#print(df_togo[key_cols].describe().T[['mean', 'std', 'min', 'max']])

#print("\nStatistics after cleaning:")
#print(df_togo_clean[key_cols].describe().T[['mean', 'std', 'min', 'max']])
...
for i, stat in enumerate(stats_to_compare):
    ...
    before_values = df_togo[key_cols].describe().T[stat]
    after_values = df_togo_clean[key_cols].describe().T[stat]
...
for i, col in enumerate(key_cols[:6]):  # Just show first 6 columns to avoid crowding
    ...
    sns.boxplot(data=[df_togo[col], df_togo_clean[col]])
...
df_togo_clean['Day'] = df_togo_clean['Timestamp'].dt.day
df_togo_clean['Hour'] = df_togo_clean['Timestamp'].dt.hour

# 1. Daily patterns - Mean values by hour of day
daily_patterns = df_togo_clean.groupby('Hour')[['GHI', 'DNI', 'DHI', 'Tamb']].mean()
...
monthly_patterns = df_togo_clean.groupby('Month')[['GHI', 'DNI', 'DHI', 'Tamb']].mean()
...
# Wind Speed vs GHI
plt.subplot(1, 3, 1)
plt.scatter(df_togo_clean['WS'], df_togo_clean['GHI'], alpha=0.5, color='blue')
...
# Wind Gust vs GHI
plt.subplot(1, 3, 2)
plt.scatter(df_togo_clean['WSgust'], df_togo_clean['GHI'], alpha=0.5, color='green')
...
plt.subplot(1, 3, 3)
plt.scatter(df_togo_clean['WD'], df_togo_clean['GHI'], alpha=0.5, color='purple')
...
# RH vs Tamb
plt.subplot(1, 2, 1)
plt.scatter(df_togo_clean['RH'], df_togo_clean['Tamb'], alpha=0.5, color='red')
...
# RH vs GHI
plt.subplot(1, 2, 2)
plt.scatter(df_togo_clean['RH'], df_togo_clean['GHI'], alpha=0.5, color='orange')
...
wd_rad = np.radians(df_togo_clean['WD'])
...
# GHI Distribution
plt.subplot(1, 2, 1)
plt.hist(df_togo_clean['GHI'], bins=30, color='orange', edgecolor='black', alpha=0.7)
plt.axvline(df_togo_clean['GHI'].mean(), color='red', linestyle='dashed', linewidth=2)
plt.text(df_togo_clean['GHI'].mean()*1.1, plt.ylim()[1]*0.9, 
         f'Mean: {df_togo_clean["GHI"].mean():.1f}', color='red')
...
plt.subplot(1, 2, 2)
plt.hist(df_togo_clean['WS'], bins=20, color='skyblue', edgecolor='black', alpha=0.7)
plt.axvline(df_togo_clean['WS'].mean(), color='blue', linestyle='dashed', linewidth=2)
plt.text(df_togo_clean['WS'].mean()*1.1, plt.ylim()[1]*0.9, 
         f'Mean: {df_togo_clean["WS"].mean():.1f}', color='blue')
...
df_togo_clean['RH_bin'] = pd.cut(df_togo_clean['RH'], 
...
plt.subplot(2, 2, 1)
scatter = plt.scatter(df_togo_clean['GHI'], df_togo_clean['Tamb'], 
                      c=df_togo_clean['RH'], cmap='viridis', alpha=0.6)
...
daily_temp_by_rh = df_togo_clean.groupby(['Hour', 'RH_bin'])['Tamb'].mean().unstack()
...
daily_ghi_by_rh = df_togo_clean.groupby(['Hour', 'RH_bin'])['GHI'].mean().unstack()
...
df_togo_clean['TempDiff_A'] = df_togo_clean['TModA'] - df_togo_clean['Tamb']
df_togo_clean['TempDiff_B'] = df_togo_clean['TModB'] - df_togo_clean['Tamb']

plt.subplot(2, 2, 4)
rh_bins = ['0-20%', '20-40%', '40-60%', '60-80%', '80-100%']
rh_counts = df_togo_clean['RH_bin'].value_counts().reindex(rh_bins)
tempdiff_by_rh = df_togo_clean.groupby('RH_bin')[['TempDiff_A', 'TempDiff_B']].mean().reindex(rh_bins)
...
sample_size = min(2000, len(df_togo_clean))
df_sample = df_togo_clean.sample(sample_size, random_state=42)
...
sample_size = min(1000, len(df_togo_clean))
df_sample = df_togo_clean.sample(sample_size, random_state=42)
